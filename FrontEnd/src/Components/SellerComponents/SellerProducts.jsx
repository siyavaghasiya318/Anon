...
const SellerProducts = () => {
    ...
    const filterdProduct = getSellerProducts.filter((product) => {
        const matchorder =
            categoryFilter === "select" || product.category === categoryFilter
            return matchorder
    })
            ...
            <div className="bg-white rounded-2xl mt-8">
                ...
                <div className="flex items-center gap-5 justify-between text-gray-400 p-5">
                    ...
                    <div className="  gap-2 bg-[#f8fafc] group hover:outline-2 transition-all duration-200 hover:outline-purple-200 hover:shadow-purple-300 flex rounded-2xl w-full items-center px-5">
                        <div><IoSearch className="text-[20px] group-hover:text-purple-500 transition-all outline-0 duration-300" /></div>
                    ...
                    <div className="gap-2 flex bg-[#f8fafc] rounded-2xl group font-bold items-center px-5 hover:outline-2 transition-all w-full duration-300 hover:outline-purple-200 hover:shadow-purple-300">
                        ...
                        <select name="" value={categoryFilter} onChange={(e) => setCategoryFilter(e.target.value)} className="py-4 outline-0  appearance-none w-full text-gray-500 text-[14px] capitalize" id="">
                                        ...
                                        <tr key={item.id} className="border-b border-gray-100 last:border-0">
                                            ...
                                            <td className="px-8 py-5">
                                                <div className="flex items-center gap-3">
                                                    <Link to={`/productdetail/${item._id}`} onClick={() => window.scrollTo(0, 0)}   ><img src={item.images[0]} className="w-11 h-11 object-cover rounded-xl bg-gray-50" alt="img" /></Link>
                                                    <div>
